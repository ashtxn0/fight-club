$(".comment-btn").on("click", function(event){
    ...
    $.ajax({
        ...
        success: function(res){
            $(".input"+fightID).val("");
            $("."+fightID+" > .comment2").remove();
            $("."+fightID+" > .comment0").before("<div class='predict-comment-container comment2'><div class='comment-col predict-comment-fill'><div class='comment-col'><div class='comment-row'><p class='comment-username'>"+ comment.username +"</p><p class='predict-time-since'>just now</p></div><p class='comment-content'>"+comment.content+"</p></div><div class='comment-row'><img id='like-comment-"+comment.username+"-"+comment.timePosted+"' class='like-btn' src='/image/thumbs-up-grey.svg' height='20px' alt=''><p class='comment-likes'>"+ comment.likes +" </p><img id='dislike-comment-"+comment.username+"-"+comment.timePosted+"' class='like-btn' src='/image/thumbs-down-grey.svg' height='20px' alt=''><p class='comment-likes'>"+ comment.dislikes +" </p></div></div></div>");
