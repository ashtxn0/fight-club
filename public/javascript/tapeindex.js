...
function showTape(i){
    ...
    if (isOpen===0){
        ...
        fighters[i].fights.forEach(function(fight){
            if(fight.tape_url){
                $(".slide-out-fighter-fights").append("<div onclick=\"openTapeUrl('"+fight.tape_url+"')\" class=\"tape-index-fight-row\"> <div class=\"tape-index-fight-outcome fight-"+fight.result+"\"> <h3>"+fight.result+"</h3> </div> <div class=\"tape-index-fight-info\"><h3 class='tape-index-fighter-name-row'>"+fighters[i].name+" <span class='tape-index-fight-date'>"+fight.date+"</span></h3> <a onclick=\"event.stopPropagation();\" target=\"_blank\" href=\"/fighter/"+fight.opponent+"\">vs. "+fight.opponent+"</a> <p>"+fight.method+"</p> </div> </div>");
            }
        })
        opponents[i].fights.forEach(function(fight){
            if(fight.tape_url){
                $(".slide-out-opponent-fights").append('<div onclick="openTapeUrl(\''+fight.tape_url+'\')" class="tape-index-fight-row"> <div class="tape-index-fight-outcome fight-'+fight.result+'"> <h3>'+fight.result+'</h3> </div> <div class="tape-index-fight-info"><h3 class="tape-index-fighter-name-row">'+opponents[i].name+' <span class="tape-index-fight-date">'+fight.date+'</span></h3> <a onclick="event.stopPropagation();" target="_blank" href="/fighter/'+fight.opponent+'">vs. '+fight.opponent+'</a> <p>'+fight.method+'</p> </div> </div>');
            }
        })
